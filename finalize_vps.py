...
def finalize():
    ...
    # Update Nginx config to use service name 'patosgym_web'
    new_conf = """
server {
    listen 80;
    ...
    location / {
        proxy_pass http://patosgym_web:8000;
...
"""
    sftp = ssh.open_sftp()
    with sftp.open('/root/Villex/nginx/conf.d/patosgym.conf', 'w') as f:
        f.write(new_conf.strip())
    sftp.close()

    print("--- RELOADING NGINX ---")
    _, out, err = ssh.exec_command('docker exec villex_nginx nginx -t && docker exec villex_nginx nginx -s reload')
    print(err.read().decode())

    print("--- VERIFYING HTTP RESPONSE CODE ---")
    _, out, _ = ssh.exec_command('docker exec villex_nginx curl -s -o /dev/null -w "%{http_code}" -H "Host: patosgym.com.ar" http://localhost')
    print("Code:", out.read().decode())
